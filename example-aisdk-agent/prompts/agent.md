...
- If the message names a thread, use that id.
- If the discussion is attached to a thread, that is the one.
- Otherwise call `list_thread_queue` or `search_threads`. You can only read or reply on a thread you
  have seen in this conversation, so search before you act.
